...
def mask(inputs, key_masks=None, type=None):
    ...
    if type in ("k", "key", "keys"):
        ...
        print('ww',inputs.get_shape())
        print('ss',key_masks.get_shape())
...
def multihead_attention(queries, keys, values, key_masks,
    ...
    '''
    d_model = queries.get_shape().as_list()
    d_model=d_model[-1]
    ...
    with tf.compat.v1.variable_scope(scope, reuse=tf.compat.v1.AUTO_REUSE):
        ...
        # print('Q_',Q_.get_shape().as_list())
        # Attention
        outputs = scaled_dot_product_attention(Q_, K_, V_, key_masks, d_model,causality, dropout_rate, training)
...
def label_smoothing(inputs, epsilon=0.1,V=100):
    ...
    ```    
    '''
    V = inputs.get_shape().as_list() # number of channels
    V=V[-1]
